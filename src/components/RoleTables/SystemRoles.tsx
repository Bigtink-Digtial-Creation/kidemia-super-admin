...
export default function SystemRoles() {
  ...
  const { data: systemRoles, isLoading } = useQuery({
    queryKey: [QueryKeys.systemRole, page],
    queryFn: () => ApiSDK.RolesService.getSystemRolesApiV1RolesSystemGet(),
  });

  const totalRoles = systemRoles?.length ?? 0;
  const totalPages = Math.ceil(totalRoles / pageSize);
  const paginatedRoles =
    systemRoles?.slice((page - 1) * pageSize, page * pageSize) || [];
  return (
    <div>
      <Table
        aria-label="system roles table"
        isStriped
        className="pt-4"
        bottomContent={
